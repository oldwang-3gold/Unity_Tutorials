...
float3 ColorBelowWater(float4 screenPos, float3 tangentSpaceNormal) {
	...
	uvOffset.y *= _CameraDepthTexture_TexelSize.z * abs(_CameraDepthTexture_TexelSize.y); // 乘以深度图宽度 * 1 / 每个纹素宽度
	
	float2 uv = AlignWithGrabTexel((screenPos.xy + uvOffset) / screenPos.w);
	float2 backgroundDepth = LinearEyeDepth(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, uv));
	...
	float3 fogFactor = exp2(-_WaterFogDensity * depthDifference);
